export const data = [
  {
    ar:
      'احترام وتقدير القوانين واللوائح المعمول بها داخل مستشفى الموسى التخصصي أثناء الزيارات والاستشارات.',
    en:
      'Respect and value the hospital rules and regulations while paying a visit or approaching for consultations.'
  },
  {
    ar:
      'التعامل مع المرضى الآخرين والأطباء والممرضين والموظفين باحترام وتقدير.',
    en:
      'Show consideration for others and deal with other patients and staff with respect.'
  },
  {
    ar: 'احترام خصوصية المرضى الآخرين وتقدير ظروفهم الخاصة.',
    en: 'Respect the privacy of other patients.'
  },
  {
    ar:
      'تزويد المستشفى بمعلومات شاملة ودقيقة وواضحة حول الشكاوى الحالية، والتي تشمل أي معلومات عن المرض السابق والخطط العلاجية المتبعة مسبقًا، وأي حساسيّة معروفة من الأدوية أو الأغذية، أو أي معلومة صحية متعلقة بالمريض.',
    en:
      'Provide complete and accurate information about the present complaints, past illness, hospitalization, treatment, and any known sensitivity to medication and any other matter relating to your health.'
  },
  {
    ar:
      'اتباع تعليمات الطاقم الطبي والطبيب المتابع للحالة بعناية ودقة دون إهمال أو تقصير.',
    en: "Follow the health care provider's instructions."
  },
  {
    ar:
      'تحمل مسؤولية رفض العلاج أو عدم اتباع خطة العلاج حتى بعد توضيح خطر عدم اتباعها.',
    en:
      'Accept responsibility for refusing treatment or not following the treatment plan even after explaining the risk of not following it.'
  },
  {
    ar: 'الالتزام الكامل بالمواعيد المتفق عليها.',
    en: ' Avoid any delay in taking appointments from the appointment office.'
  },
  {
    ar:
      'حضور الموعد في الوقت المحدد، وإذا رغب المريض في إلغائه، عليه إبلاغ قسم الموعد قبل 48 ساعة.',
    en:
      'Attend the appointment on time and if you want to cancel it, inform the appointment section 48 hours in advance.'
  },
  {
    ar: 'احترام الأولوية المعطاة للحالات الطارئة والمستعجلة.',
    en: 'Respect the priority given to emergency cases.'
  },
  {
    ar:
      'يتحمل المرضى المسؤولية في مساعدة العاملين في منشآت المستشفى في التحكم في الضوضاء والتدخين ومراقبة الزائرين.',
    en:
      "The patient is responsible for his role in assisting the ASH facility staff in the control of excessive noise, smoking, and visitor's regulation and rights."
  },
  {
    ar: 'الالتزام بسداد الالتزامات المالية نظير الرعاية الطبية المقدمة للمريض.',
    en:
      'Ensure that financial obligations of your health care are fulfilled promptly (If it exists).'
  },
  {
    ar:
      'تقديم العينات المطلوبة وإجراء الفحوصات الطبية في الوقت المحدد دون تأخير.',
    en:
      'Give the requested samples and make the medical checkups on time with no delay.'
  },
  {
    ar:
      'في الحالات الطارئة، تسجل المتعلقات الشخصية الثمينة الخاصة بالمريض في إيصال المتعلقات الثمينة للمرضى من قبل الممرضة بحضور المريض والمدير المناوب والحصول على توقيع المريض أو أحد مرافقيه كسياسة وإجراءات قيمة للمريض.المستشفى غير مسؤول عن فقدان المتعلقات الشخصية الثمينة داخله، لذلك نرجو منكم تجنب إحضارها إلى المستشفى.',
    en:
      "Avoid bringing valuable personal things to the hospital since the hospital is not Responsible for keeping them. However, in any emergency cases and unaccompanied admissions, you will have their valuables recorded in the Patients' Valuable Receipt by the nurse in the presence of the patient and manager on duty and obtain their signature as patient's valuable policy & procedures."
  },
  {
    ar:
      'يتحمل المريض و/أو أسرته مسؤولية احترام ممتلكات منشأة مستشفى الموسى التخصصي والأشخاص الآخرين داخل المستشفى.',

    en:
      'The patient and his/her family are responsible for being respectful of the property of the ASH facility and other persons in the healthcare facility.'
  }
];
