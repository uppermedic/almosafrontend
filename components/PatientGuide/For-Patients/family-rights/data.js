export const data = [
  {
    rights: [
      {
        num: '01',
        ar: {
          content: `
          فور دخول المريض لمستشفى الموسى التخصصي فإنه سيحصل على نسخة مكتوبة من اللوائح والقوانين من قسم الاستقبال -مكتب التسجيل-  طاقم التمريض، وفي حال واجهت المريض أي مشاكل في فهم أيًا من هذه اللوائح، فإنه يمكنه الاتصال على مكتب علاقات المرضى، لتوفير المساعدة فورًا له وعمل اللازم.
          `
        },
        en: {
          content: `
          You have the right to understand and use all your rights: (You will receive a written copy of these rights from the Reception, Registration Office or Nursing Staff). If, for any reason, you did not understand them, please contact Patient Relation Office asks for help including an interpreter.
          `
        }
      },
      {
        num: '02',
        ar: {
          content: `
          نؤمن بحق الجميع بالحصول على رعاية طبية تتسم بالمساواة واحترام معتقدات المريض وقيمه الشخصية، والذي على كافة موظفي مستشفى الموسى التخصصي تقديم الرعاية الصحية دون أي تفرقة حسب القوانين ولوائح المستشفى والمملكة العربية السعودية.
          `
        },
        en: {
          content: `
          You have the right to receive impartial care without discrimination, including a request for pastoral services or similar request respecting your values and spiritual I religious beliefs from all authorized hospital staff. This shall be subject to availability without interfering with other patient's safety, ASH work environment, and without contradiction with ASH rules & regulations.
          `
        }
      },
      {
        num: '03',
        ar: {
          content: `
          من حق المريض داخل مستشفى الموسى التخصصي الحصول على عناية طبية كاملة ومتقنة من أجل الوصول إلى تشخيص طبي صحيح حسب الأعراف الطبية المعمول بها، والوصول لاحقًا إلى العلاج المناسب.
          `
        },
        en: {
          content: `
          You have the right to receive comprehensive Medical Care to reach the proper diagnosis and treatment of illness and injury.
          `
        }
      },
      {
        num: '04',
        ar: {
          content: `
          لدى المريض الحق في الحصول على الرعاية الطارئة والفورية كلما دعت الحاجة إلى ذلك، دون تكاسل أو تقصير من طاقم التمريض أو الأطباء.
          `
        },
        en: {
          content: `
          You have the right to know the identity of the physician or other practitioner responsible for your care.
          `
        }
      },
      {
        num: '05',
        ar: {
          content: `
          لدى المريض الحق في معرفة هوية الطبيب المعالج والطاقم الطبي المتابع للحالة من ممرضين أو إداريين أو مشرفيين.
          `
        },
        en: {
          content: `
          You have the right to receive comprehensive information in a manner you can understand (from the attending physician) about the diagnosis, proposed treatment, any change in your health status, and the cause of these changes. Alternative treatments, potential benefits and drawbacks to the proposed treatment, likelihood of successful treatment, possible problems related to recovery, and possible results of non-treatment.
          `
        }
      }
    ],
    img: '/images/patient-guide/for_patient/patient_rights_1.jpg'
  },
  {
    rights: [
      {
        num: '06',
        ar: {
          content: `
          الحصول على معلومات كاملة بطريقة سلسة ومفهومة من خلال الطبيب المعالج والمتابع للحالة هو من حق المريض الكامل بمجرد تشخيص المريض، هذه المعلومات قد تشمل:
          <ul>
            <li>
            النظام العلاجي المقترح.
            </li>
            <li>
            أي تغييرات قد تطرأ على الحالة الصحية للمريض وأسبابها.
            </li>
            <li>
            العلاجات البديلة والممكنة.
            </li>
            <li>
            احتمالية نجاح العلاج أو نسبة فشله.
            </li>
            <li>
            ميزات العلاج والمخاطر (إن وجدت).
            </li>
            <li>
            المضاعفات المتوقعة أثناء العلاج.
            </li>
            <li>
            النتائج المتوقعة في حالة إهمال العلاج وعدم الأخذ بنصائح الطبيب المعالج.
            </li>
          </ul>
          `
        },
        en: {
          content: `
          You have the right to have an interpreter (if available) if the language barrier presents a problem of understanding the details of your care and the treatment provided to you.
          `
        }
      },
      {
        num: '07',
        ar: {
          content: `
          إذا شكلت اللغة عائقًا دون فهم التعليمات الطبية أو عانى المريض من صعوبة في التواصل مع الطاقم الطبي أو الإداري، فإنه يحق له الحصول على مترجم – إن وجد.
          `
        },
        en: {
          content: `
          You have the right to give your written General Consent for treatment when you enter the hospital.
          `
        }
      },
      {
        num: '08',
        ar: {
          content: `
          يرجى إعطاء الموافقة على العلاج في المستشفى كتابيًا من قبل المريض أو من قبل ذويه بمجرد دخول المريض إلى مستشفى الموسى التخصصي.
          `
        },
        en: {
          content: `
          You have the right to give your written consent before surgical, invasive procedures, anesthesia, and the use of blood and its products, and any other high-risk procedures and treatments, and for research participation.
          `
        }
      },
      {
        num: '09',
        ar: {
          content: `
          يرجى من المرضى إعطاء موافقة كتابية مسبقة من قبل أن تجرى له أي عملية جراحية أو منظار أو تخدير، أول نقل الدم ومكوناته أو أيّة إجراءات أخرى تتطلب ذلك الإجراء مثل المشاركة في الأبحاث العلمية.
          `
        },
        en: {
          content: `
          You have the right to receive all information that you needed before giving your consent for any proposed procedure or treatment. This information should include the benefits, available alternatives, and the possible risks of the procedure or the treatment.
          `
        }
      },
      {
        num: '10',
        ar: {
          content: `
          للمريض الحق في الحصول على جميع المعلومات والتفاصيل التي يحتاجها قبل الموافقة على خطة العلاج أو الإجراء المقترح، والتي تشمل:
          <ul>
            <li>
            الفوائد من خطة العلاج أو الإجراء المقترح.
            </li>
            <li>
            البدائل المتوفرة لهذا الإجراء.
            </li>
            <li>
            المخاطر المحتملة والمضاعفات المتوقعة بعد الإجراء المقترح.
            </li>
          </ul>
          `
        },
        en: {
          content: `
          You have the right to participate in care decisions. ASH encourages Patients, Parents, or Legal Guardians to participate in planning and implementing treatment with nurses and physicians to the extent they wish.
          `
        }
      }
    ],
    img: '/images/patient-guide/for_patient/patient_rights_2.jpg'
  },
  {
    rights: [
      {
        num: '11',
        ar: {
          content: `
          لدى المريض أو من ينوب عنه الحق في مشاركة الفريق الطبي في اتخاذ القرارات الصحية المرتبطة بحالة المريض، يأتي هذا من باب تشجيع مستشفى الموسى التخصصي وذويهم على اتخاذ القرارات المناسبة لتحقيق الراحة الكاملة للمريض وذويه.
          `
        },
        en: {
          content: `
          You have the right to maintain your need for privacy for all examinations, procedures, clinical interviews, transport, treatments, and the confidentiality of all of your information.
          `
        }
      },
      {
        num: '12',
        ar: {
          content: `
          جميع الفحوصات والإجراءات والمقابلات السريرية ونقل الأعضاء والعلاج داخل مستشفى الموسى التخصصي هي معلومات سرية خاصة بالمريض لا يحق لأحد الاطلاع عليها أو معرفتها إلا أصحاب الشأن.
          `
        },
        en: {
          content: `
          You have the right to choose the treating physician (depending on availability) in addition to the right to reject the treating physician upon furnishing a genuine reason. Also, you have the right to ask for a second opinion without fear of compromise, in case of complications expected or unexpected, unsatisfied results of treatment, or in making a risky decision in management. In addition, you have the choice to have face-to-face consultation or virtual consultation through the ASH application.
          `
        }
      },
      {
        num: '13',
        ar: {
          content: `
          يحق للمريض اختيار الطبيب المعالج لحالته من الأطباء المتوفرين، والاعتراض على الطبيب المعالج بعد إبداء الأسباب المقنعة لهذا الطلب، كما يحق له استشارة طبيب آخر دون أدنى اعتراض من إدارة مستشفى الموسى التخصصي طبقًا للأنظمة واللوائح الداخلية للمستشفى، في حالة حدوث مضاعفات متوقعة أو غير متوقعة أو تحقيق نتائج غير مرضية للعلاج.
          `
        },
        en: {
          content: `
          You have the right to refuse the treatment (if you would like to) and be informed of the medical consequences due to your refusal. Accordingly, you will sign a form prepared to that effect and leave the hospital.
          `
        }
      },
      {
        num: '14',
        ar: {
          content: `
          يمكن للمريض اختيار الاستشارة وجهًا لوجه، أو الاستشارة الافتراضية من خلال تطبيق مستشفى الموسى التخصصي.
          `
        },
        en: {
          content: `
          You have the right to refuse examination or observation by a trainee not directly responsible for your care. In such cases, they should have the permission of the patient to be present during the case discussion, consultation, examination, and treatment.
          `
        }
      },
      {
        num: '15',
        ar: {
          content: `
          يحق للمريض رفض العلاج إذا رغب في ذلك، حينها يجب إعلام المريض أو ذويه بالتبعات الصحية لهذا الرفض، وعليه سوف يوقع على إقرار معد من قبل إدارة المستشفى لهذا الغرض، ومغادرة المستشفى بعدها.
          `
        },
        en: {
          content: `
          You have the right to be protected from physical, verbal, and psychological assault.
          `
        }
      }
    ],
    img: '/images/patient-guide/for_patient/patient_rights_3.jpg'
  },
  {
    rights: [
      {
        num: '16',
        ar: {
          content: `
          يحق للمريض رفض الكشف عليه أو متابعة تفاصيل علاجه من قبل أي شخص ليس مسئولاً عن علاجه بشكل مباشر، ويتعين على الذين لا يشاركون في علاجه بشكل مباشر أخذ الإذن المسبق منه حتى يتمكنوا من الحضور خلال مناقشة حالته المرضية والكشف عليه والإشراف على علاجه.
          `
        },
        en: {
          content: `
          You have the right to receive information from the organization that supports your choice to donate organs and other tissues. This information includes potential benefits and Risks. Call Toll-free number #8001245500, and you will receive the full knowledge that supports your choice from the Saudi Center for Organ Donation.
          `
        }
      },
      {
        num: '17',
        ar: {
          content: `
          لا يمكن أن يرضى مستشفى الموسى التخصصي بالاعتداء على أي مريض ماديًا أو لفظيًا أو نفسيًا، وللمريض الحق في الشكوى لإدارة المستشفى إذا ثبت ذلك.
          `
        },
        en: {
          content: `
          According to Hospital Policies and Procedures, depending on availability and your health status, you have the right to have a family member or a guardian or care partner as a companion (if needed). However, specific units will not allow this due to some medical reasons, such as ICU, Nursery, etc.
          `
        }
      },
      {
        num: '18',
        ar: {
          content: `
          للمريض الحق في تلقي المعلومات والتفاصيل التي تدعم اختياره للتبرع بالأعضاء أو الأنسجة الأخرى، وهذه المعلومات تتضمن المنافع والمخاطر المحتملة والمتوقعة من الإجراءات الموافق عليها، ويتم ذلك عن طريق الجمعية السعودية للتبرع بالأعضاء (هاتف رقم 800124500).
          `
        },
        en: {
          content: `
          You have the right to receive a complete explanation of the need to be transferred to another Treatment Center and all reasonable alternatives to such a transfer. The
          The institution to which the patient is to be transferred must accept the patient before being transferred.
          `
        }
      },
      {
        num: '19',
        ar: {
          content: `
          وفقًا لسياسات وإجراءات المستشفى واعتمادًا على حالة المريض الصحية، يحق للمريض الحصول على فرد من العائلة أو وصي أو شريك رعاية كمرافق (إذا لزم الأمر) في بعض الأقسام المسموح للمريض بذلك، في حين أنه لا يمكن ذلك في أقسام معينة أخرى مثل العناية المركزة والحضانة، لأسباب طبية متعلقة بالإجراءات وحساسية الموقف.
          `
        },
        en: {
          content: `
          You have the right to complain at any time through Patient Relation Officer (contact internal number: 1423, mobile: 0552952660) without any fear of reprisals about the care and services you are receiving, and the hospital should respond to you. Whether inpatients and outpatients, patients can contact Personal Relation Office to give complaints and suggestions about services and have them addressed to the concerned department.
          `
        }
      },
      {
        num: '20',
        ar: {
          content: `
          في حالة نقل المريض إلى مركز علاجي آخر، فإنه يحق له الحصول على توضيح شامل عن الأسباب وكافة الالتزامات المترتبة على ذلك ماديًا وإداريًا أن وجدت، وتحديد الاختيارات البديلة عن ذلك، عندها سترتب إدارة المستشفى كافة الإجراءات المتعلقة بالحصول على موافقة المركز العلاجي البديل لاستقبال المريض، وذلك قبل الشروع في عملية النقل.
          `
        },
        en: {
          content: `
          You have the right to view your medical record under the doctor's supervision or the medical team, who is in charge of your treatment and according to the hospital policy & regulations, through the office of patient relation.
          `
        }
      }
    ],
    img: '/images/patient-guide/for_patient/patient_rights_4.jpg'
  },
  {
    rights: [
      {
        num: '21',
        ar: {
          content: `
          يمكن للمريض تقديم أي استفسار أو اقتراح أو شكوى عن طريق مكتب علاقات المرضى (تحويلة 1423) جوال: 0552952660، دون الشعور بالخوف أو القلق حيال تأثر الخدمة والرعاية المقدمة للمريض، كما يحق للمريض الحصول على رد من إدارة مستشفى الموسى التخصصي بشأن الشكوى المقدمة.
          `
        },
        en: {
          content: `
          The organization respects and supports patient's right to appropriate assessment and pain management and provides education for patients and their families about pain and pain management.
          `
        }
      },
      {
        num: '22',
        ar: {
          content: `
          يحق للمريض الاطلاع على ملفه الطبي تحت إشراف الطبيب المعالج أو أحد أفراد الطاقم الطبي وذلك حسب النظام والقوانين المتبعة في المستشفى عن طريق مكتب علاقات المرضى.
          `
        },
        en: {
          content: `
          ASH is committed to providing (End of life care) for the dying patient with Respectful and compassionate care and respects his unique needs.
          `
        }
      },
      {
        num: '23',
        ar: {
          content: `
          يلتزم المستشفى بمعاملة المرضى ذوي المراحل المتقدمة والصعبة من المرض معاملة كريمة وفق الأسس العالمية المتبعة في العلاج التلطيفي وتخفيف الألم، بما في ذلك احترام الاحتياجات الاستثنائية لهم ولأسرهم.
          `
        },
        en: {
          content: `
          You have the right to obtain a Medical Reports and a copy of the Medical test results from the Medical Reports Section (if you would like to) for which the hospital at no additional charges; however, this will only be applied for the 1st time for which the hospital will charge you.
          `
        }
      },
      {
        num: '24',
        ar: {
          content: `
          يمكن للمريض طلب تقرير طبي ونسخة من التحاليل من خلال قسم السجلات الطبية، من دفع أي رسوم في المرة الأولى للطلب، وبعد ذلك يتم تحصيل الرسوم المقررة من قبل المستشفى.
          `
        },
        en: {
          content: `
          You have the right to receive an itemized bill and explanation of all charges regardless it is paid by the patient or by another source.
          `
        }
      },
      {
        num: '25',
        ar: {
          content: `
          يحق للمريض الحصول على فاتورة مفصلة توضح كامل التكاليف، بغض النظر عن إذا ما كانت الفاتورة ستدفع من قبل الجهة الأخرى.
          `
        },
        en: {
          content: `
          You have the right to receive written Discharge Instructions or Plans from the nursing team. This plan will highlight treatment follow-up and medical instructions on how to take care of yourself.
          `
        }
      }
      // {
      // 	num: '26',
      // 	ar: {
      // 		content: `
      //     عند خروج المريض من مستشفى الموسى التخصصي يحصل على إرشادات الخروج من قبل فريق التمريض، والتي تحتوي على الخطة العلاجية والنصائح الطبية اللازمة للعناية الكاملة للمريض.
      //     `
      // 	},
      // 	en: {
      // 		content: `
      //     When the patient is discharged from Al-Moosa Specialist Hospital, he obtains exit instructions from the nursing team, which contain the treatment plan and medical advice necessary for the patient's full care.
      //     `
      // 	}
      // }
    ],
    img: '/images/patient-guide/for_patient/patient_rights_5.jpg'
  }
];

const secondRights = [
  {
    num: '01',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '02',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '03',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '04',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '05',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '06',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '07',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '08',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '09',
    content:
      'Lorem ipsum dolor sit aLorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!!'
  },
  {
    num: '10',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '11',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '12',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '13',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  },
  {
    num: '14',
    content:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta, minima!'
  }
];
